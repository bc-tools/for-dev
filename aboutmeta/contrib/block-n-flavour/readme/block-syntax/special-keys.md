...
A little DSL allows you to define some special features for keys. Here's what you can use.
  ...
  1. Non-competing keys that use the same parser, whether optional or required, can be quickly specified by separating them with commas (which avoids having to type them one after the other). For example, `key-1, key-2 * : same_parser` defines optional keys that can be used together and rely on the same parser.
