...
> ***WARNING.*** *The purpose of a data type class is limited to storing data via attributes, optionally performing data normalization (e.g., standardizing a valid URL format) and data validation when needed (e.g., verifying if a physical address exists). Parsing user input is the responsibility of parsers, not data objects.*
